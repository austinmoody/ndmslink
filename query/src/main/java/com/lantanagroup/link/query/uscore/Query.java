...
public class Query extends BaseQuery implements IQuery {
  ...
  @Override
  public void execute(ReportCriteria criteria, ReportContext context, List<PatientOfInterestModel> patientsOfInterest, String reportId, List<String> resourceTypes, String measureId) {
    ...
    if (patientsOfInterest.size() > 0) {
      try {
        ...
        scoop.execute(criteria, context, patientsOfInterest, reportId, resourceTypes, measureId);
      } catch (Exception ex) {
